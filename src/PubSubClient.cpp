...
/* Concept of the method changed. It returns true if MQTT Message is received */
      ...
      if (len > 0) {
        ...
        if (type == MQTTPUBLISH) {
          ...
          */
            ...
            memmove(buffer + llen + 2, buffer + llen + 3,
...
boolean PubSubClient::publish(const char *topic, const char *payload) {
  return publish(topic, (const uint8_t *)payload, payload ? strlen(payload) : 0,
                 false);
}
/* unused code
boolean PubSubClient::publish(const char *topic, const char *payload,
...
boolean PubSubClient::beginPublish(const char *topic, unsigned int plength,
  ...
  if (connected()) {
    ...
    size_t hlen =
        buildHeader(header, buffer, plength + length - MQTT_MAX_HEADER_SIZE);
